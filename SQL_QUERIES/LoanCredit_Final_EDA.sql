...
2. What is the minimum applicants income?
3. What is the maximum applicants income?
4. What is the average credit score for our dataset
5. What is the min credit score for our dataset
6. What is the max credit score for our dataset
...
ROUND(STDDEV(Income), 0) as STD_Income
FROM loancredit_data;

SELECT 
ROUND(AVG(creditscore), 0) as Average_CreditScore,
ROUND(MIN(creditscore), 0) as MIN_CreditScore,
ROUND(MAX(creditscore), 0) as MAX_CreditScore,
ROUND(STDDEV(creditscore), 0) as STD_CreditScore
FROM loancredit_data;

SELECT 
ROUND(AVG(loanamount), 2) as Average_LoanAmount,
ROUND(MIN(loanamount), 2) as MIN_LoanAmount,
ROUND(MAX(loanamount), 2) as MAX_LoanAmount,
ROUND(STDDEV(loanamount), 0) as STDLoanAmount
FROM loancredit_data;
...
SELECT 
	loanApproved,
    ROUND(AVG(loanAmount), 2) AS AverageApprovedLoanAmount
FROM LOANCREDIT_DATA
GROUP BY loanApproved;
...
GROUP BY EMPLOYMENTTYPE
ORDER BY AverageApprovedLoanAmount DESC;

/*===========================================================
SECTION 5: CREDIT RISK ANALYSIS 
